 ...
 * @example experienceForLevel(2) // Returns 100
 * @example experienceForLevel(10) // Returns 45000
 */
export const experienceForLevel = (level: number): number => {
  if (level <= 1) return 0;
  // Using the formula from the original frontend SkillSystem.ts
  // Level N requires sum(i=1 to N-1) of floor(i + 300 * 2^(i/7)) / 4 - RS Classic formula
  // Using simpler formula: Total XP = floor(level * (level - 1) * 50)
 ...
 * @example levelFromExperience(0) // Returns 1
 * @example levelFromExperience(150) // Returns 2
 * @example levelFromExperience(50000) // Returns 10
 */
